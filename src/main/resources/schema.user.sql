create table board ( board_id integer auto_increment, board_name varchar(50) not null, description varchar(255), creation timestamp not null,currency varchar(3),`updated` timestamp not null, primary key (board_id), key `updated_idx` (`updated`));
...
alter table item add constraint `amount_non_zero` check ((`amount` <> 0)); 
